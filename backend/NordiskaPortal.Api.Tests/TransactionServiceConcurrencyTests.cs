...
namespace NordiskaPortal.Api.Tests
{
    ...
    public class TransactionServiceConcurrencyTests
    {
        ...
        {
            // Arrange: account 1 (Anna's NKM-10001) is seeded with a single 125000 deposit via the real migration's HasData.
            // Two withdrawals of 70000 each are individually valid (70000 < 125000) but together (140000) would overdraw the account.
            const int accountId = 1;
            const decimal withdrawAmount = 70000m;

            // Two separate DbContext instances and two separate TransactionService instances
            // This mirrors two real, independent HTTP requests, each with their own scoped DbContext, rather than one shared context.
            ...
            // The failure should be the specific concurrency-conflict message, not "Otillräckligt saldo"
            // that distinction is what proves the Serializable isolation caught a genuine overlapping conflict,
            // rather than the second request simply seeing an already-reduced balance from a sequential (non-overlapping) execution.
            var failure = results.First(r => !r.Success);
            Assert.Contains("samtidig åtkomst", failure.Error);

            // Regardless of which specific message won the race, the balance itself must reflect exactly one withdrawal.
            // This is the actual correctness guarantee, independent of the message assertion above.
        ...
        {
            // Deposits should not race with each other at all.
            // This test confirms that two concurrent deposits both succeed cleanly,
            // contrasting with the withdrawal test above where one is expected to fail. 
 
            //Uses account 2 (also seeded, 45000 opening balance) to stay independent of the withdrawal test's account.
